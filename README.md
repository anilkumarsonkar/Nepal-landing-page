# Global Care Health — Nepal Landing Page

A premium, conversion-focused, country-specific medical-tourism landing page for
**patients from Nepal 🇳🇵 seeking medical treatment in India 🇮🇳**, built for
**Global Care Health**.

Goal: attract Nepali patients and convert them into leads via enquiry form,
...
Sticky header · Hero (BD→IN route) · Trust stats · Why Nepali patients
...
(incl. video support) · Nepal-specific contact block · FAQ accordion ·
...
| Nepal contact block | Phone, WhatsApp, office, hours, emergency |
...
- Suggested URL: `/medical-treatment-in-india/nepal`

Primary keyword: *Medical Treatment in India for Nepal Patients*.
...
- Live URL: https://4rx.co/nepal/ (BMT: /nepal/bmt · Cardiac: /nepal/cardiac)
- Server path: `/home/rx/public_html/nepal/` on 161.97.108.25 (cPanel user `rx`)
- Form endpoint: `lead.php` → emails each case (with uploaded reports) to **enquiry@globalcarehealth.com**
  from `leads@4rx.co`, and keeps a backup in `/home/rx/storage/nepal_leads/` (JSON lines + files).
- Redeploy: upload the folder contents (excluding `.git`, `README.md`, `vercel.json`) to the server path, then
  `chown -R rx:rx /home/rx/public_html/nepal`.
