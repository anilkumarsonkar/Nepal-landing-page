/* ============================================================
   GLOBALCARE HEALTH — NEPAL LANDING PAGE (3 specialty variants)
   ...
   8. Nepali / English toggle (Google Translate, bn + en only)
   ============================================================ */
(function () {
  'use strict';
  ...
  var VARIANTS = {
    oncology: {
      ...
      title: 'Cancer Treatment in India for Nepal Patients | GlobalCare',
      description: 'Explore cancer treatment options in India for patients from Nepal. Get specialist and hospital coordination, indicative costs, Nepali support, travel and arrival guidance.',
      wa: "Hello GlobalCare Health, I'm contacting you from Nepal about cancer treatment in India for [myself / a family member]. I'd like to share the medical reports and understand the next steps."
    },
    bmt: {
      cta: 'Get My Case Reviewed',
      ctaShort: 'Get My Case Reviewed',
      formTitle: 'Share Your BMT Reports',
      title: 'Bone Marrow Transplant in India for Nepal Patients | GlobalCare Health',
      description: 'Bone marrow transplant (BMT) and blood cancer treatment in India for patients from Nepal. Share your reports for a haematology/BMT team review, an indicative BMT cost in India and next steps — before you travel.',
      wa: "Hello GlobalCare Health, I'm contacting you from Nepal about a bone marrow transplant / blood disorder treatment in India for [myself / a family member]. I'd like to share the reports and understand the next steps."
    },
    cardiac: {
      cta: 'Get My Case Reviewed',
      ctaShort: 'Get My Case Reviewed',
      formTitle: 'Share Your Cardiac Reports',
      title: 'Heart Surgery in India for Nepal Patients | GlobalCare Health',
      description: 'Heart surgery in India for patients from Nepal — bypass (CABG), valve replacement and complex cardiac procedures. Share your angiogram and echo for a cardiac surgeon review, an indicative cost and next steps — before you travel.',
      wa: "Hello GlobalCare Health, I'm contacting you from Nepal about heart surgery in India for [myself / a family member]. I'd like to share the cardiac reports (angiogram / echo) and understand the next steps."
    }
  };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (langToggle) {
      ...
      var setCookieLang = function (lang) {
        ...
        // Google Translate stores "googtrans" on the host AND on the parent domain (e.g. .globalcarehealth.com).
        // Clear every variant first; a single leftover cookie keeps the page translated after "English" is clicked.
        var domains = [''];
        ...
        domains.forEach(function (d) {
          document.cookie = 'googtrans=;path=/' + (d ? ';domain=' + d : '') + expired;
        });
        if (lang !== 'en') {
          var val = '/en/' + lang;
          document.cookie = 'googtrans=' + val + ';path=/';
          if (host && host.indexOf('.') > -1) { document.cookie = 'googtrans=' + val + ';path=/;domain=' + host; }
        }
      };
      var gtLoaded = false;
      var loadGoogleTranslate = function () {
        ...
        window.googleTranslateElementInit = function () {
          try {
            new window.google.translate.TranslateElement({ pageLanguage: 'en', includedLanguages: 'ne,en', autoDisplay: false }, 'google_translate_element');
          } catch (e) {}
        };
        var s = document.createElement('script');
    ...
    /* ---------- NEPAL: hero page turn (photo first, then the form) ---------- */
    var heroFlip = document.getElementById("heroFlip");
    if (heroFlip) {
      var flipDelay = parseInt(heroFlip.getAttribute("data-flip-delay"), 10) || 2800;   // + 1.1 s turn = under 4 s
      var flipDone = false;
      var doFlip = function () {
        if (flipDone) { return; }
        flipDone = true;
        heroFlip.classList.add("is-flipped");
        setTimeout(function () { heroFlip.classList.add("is-done"); }, 1300);
      };
      var flipPage = heroFlip.querySelector(".hero-flip-page");
      if (flipPage) {
        flipPage.addEventListener("click", doFlip);
        flipPage.addEventListener("keydown", function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); doFlip(); } });
      }
      // Any CTA that jumps to the form turns the page immediately
      document.querySelectorAll("a[href=\"#share-reports\"]").forEach(function (a) { a.addEventListener("click", doFlip); });
      if (location.hash === "#share-reports") { doFlip(); } else { setTimeout(doFlip, flipDelay); }
    }
