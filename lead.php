<?php
/**
 * GlobalCare Health — Nepal landing page lead endpoint
 * ------------------------------------------------------------
 ...
 *
 * Deployed at: https://4rx.co/nepal/lead.php
...
const LEAD_FROM_NAME = 'GlobalCare Nepal Landing Page';
const MAX_FILES      = 8;
...
const STORE_DIR      = '/home/rx/storage/nepal_leads';
...
// Normalise WhatsApp number: digits only, prefix +977 if the visitor typed a local number
$digits = preg_replace('/\D+/', '', $data['whatsapp']);
if (strlen($digits) >= 9 && strlen($digits) <= 11 && !str_starts_with($digits, '977')) {
    $digits = '977' . ltrim($digits, '0');
}
...
$subject = sprintf('[Nepal Lead — %s] %s, %s · %s', $variantLabel, $data['patientName'], $data['city'], $data['whatsapp_e164']);
...
$rows = [
    ...
    'Source'                    => ($_SERVER['HTTP_REFERER'] ?? 'https://4rx.co/nepal/'),
];

$esc = fn($s) => htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
$html  = '<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#16263f">';
$html .= '<h2 style="margin:0 0 4px;color:#165699">New case from the Nepal landing page</h2>';
...
$text  = "New case from the Nepal landing page\n\n";
foreach ($rows as $k => $v) {
...
$html .= '<p style="margin-top:16px;font-size:12px;color:#7c8a93">Sent automatically by the landing page at 4rx.co/nepal. Medical information is confidential — share only with the reviewing specialist.</p></div>';
...
if (!$sent) {
    error_log("[nepal lead] mail() failed for ref {$ref}");
